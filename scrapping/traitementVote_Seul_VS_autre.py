...
def traitementVote(csv, fichier_vote , numeroVote):
    ...
    for i in range(1,len(csv)):
        ...
        if csv[i][numeroVote+10] == "1" or csv[i][numeroVote+10] == "2" :
            
            if csv[i][numeroVote+10] == "1" :
                vote.append(int(csv[i][numeroVote+10]))
...
if __name__ == "__main__":
    ...
    repo_dir = "analyse/data_seul_VS_autre"
    os.chdir(repo_dir)
    
    chargement_fichier_vote(fichier,fichier_vote,4)
